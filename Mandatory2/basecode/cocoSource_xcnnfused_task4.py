...
class imageCaptionModel(nn.Module):
    def __init__(self, config):
        ...
        if True == self.simplifiedrnn:
          ...
          self.rnn = RNN(input_size=self.embedding_size  + self.nnmapsize , hidden_state_size=self.hidden_state_sizes, last_layer_state_size = self.hidden_state_sizes+10, num_rnn_layers=self.num_rnn_layers, cell_type=self.cell_type)
    ...
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, last_layer_state_size, cell_type='GRU'):
       ...
       #TODO
        ...
        for insiz in input_size_list:
            self.cells.append(LSTMCell(self.hidden_state_size, insiz))
...
class LSTMCell(nn.Module):
    ...
    def forward(self, x, state_old):
        ...
        cell_state = gate_forget*state_old+gate_input*gate_meminput
        hiddenstate = gate_output*torch.tanh(cell_state)

        state_new     = hiddenstate
        return state_new
