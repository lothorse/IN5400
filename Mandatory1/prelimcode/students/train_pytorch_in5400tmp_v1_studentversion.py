...
def evaluate_meanavgprecision(model, dataloader, criterion, device, numcl):
    ...
    concat_pred=[np.empty(shape=(0)) for _ in range(numcl)] #prediction scores for each class. each numpy array is a list of scores. one score per image; Changed the array size
    concat_labels=[np.empty(shape=(0)) for _ in range(numcl)] #labels scores for each class. each numpy array is a list of labels. one label per image; Changed the array size
    ...
    with torch.no_grad():
      ...
      for batch_idx, data in enumerate(dataloader):
          ...
          for clIndex in range(numcl):
              concat_pred[clIndex] = np.append(concat_pred[clIndex], cpuout.numpy()[:,clIndex])
              concat_labels[clIndex] = np.append(concat_labels[clIndex], labels.numpy()[:,clIndex])
...
def traineval2_model_nocv(dataloader_train, dataloader_test ,  model ,  criterion, optimizer, scheduler, num_epochs, device, numcl):
  ...
  for epoch in range(num_epochs):
    ...
    print('at epoch: ', epoch,' classwise perfmeasure \n', perfmeasure)
...
def runstuff():
  ...
  config['maxnumepochs'] = 80 #140 70 35
